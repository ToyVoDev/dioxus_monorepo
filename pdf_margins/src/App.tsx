  ...
  useEffect(() => {
    if (originalBytes) {
      centerPdf(originalBytes, {
        ...
        paperSize: Object.entries(PageSizes).find(
          (value) => value[0] === paperSize,
        )?.[1],
      }).then((centeredBytes) => {
  ...
  return (
    <Stack component="main" sx={{ height: "100%" }} spacing={2}>
      ...
      <Grid container sx={{ paddingLeft: 2, gap: 2 }}>
        ...
        <FormControlLabel
          label="Draw Alignment Corner"
          control={
            <Checkbox
              checked={drawAlignment}
              onChange={(e) => setDrawAlignment(e.target.checked)}
            />
          }
        />
        <FormControlLabel
        ...
        <TextField
          type="number"
          label="Nudge Border X"
          value={nudgeBorderWidth}
          onChange={(e) => setNudgeBorderWidth(e.target.value)}
        />
        <TextField
          type="number"
          label="Nudge Border Y"
          value={nudgeBorderHeight}
        ...
        >
          {Object.keys(PageSizes).map((paperSizeName, i) => (
            <MenuItem key={i} value={paperSizeName}>
              {paperSizeName}
            </MenuItem>
          ))}
        </TextField>
