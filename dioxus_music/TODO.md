...
- [ ] **1. Theme + Layout Shell** — Adopt `kinetic_ui` in `packages/ui`, restyle AppShell/Sidebar/PlayerBar/Navbar, responsive breakpoints (web/desktop collapse to mobile-like at narrow widths), mobile bottom nav
- [ ] **2. Library + Album Detail views** — Restyle library grid, album cards, tracklist table, album header with asymmetric layout. Based on good Stitch mockups (`library_overview/`, `album_detail/`)
- [ ] **3. Now Playing view** — Full-screen player with album art, controls, progress bar, technical metadata, queue panel. Design from scratch (Stitch mockup was bad). Reference `now_playing/code.html` for intended content, ignore layout
